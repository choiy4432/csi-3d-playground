...
- [ ] 시나리오 기본 정보 페이지 (SCENARIO + CONFIG — grade_band · damage_rate · suspect_count · time)
- [ ] NPC 관리 페이지 (목록 CRUD · npc_kind · name)
- [ ] 정답 설정 페이지 (SOLUTION — 범인 지정 · correct_inference · SOLUTION_CLUE 단서 체인)
- [ ] 증거물 카탈로그 페이지 (EVIDENCE_DEF — GLB 파일 · colliderSize · miniGame 설정)
- [ ] 생성 슬롯 정의 페이지 (GENERATION_SLOT · GENERATION_CONSTRAINT · fallback_payload)
